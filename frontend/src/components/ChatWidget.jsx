        ...
        {open && (
          ...
          >
            <div className="flex items-center justify-between px-4 py-3.5 bg-gradient-to-r from-amber/20 to-transparent border-b border-primary/10">
              ...
              <button onClick={() => setOpen(false)} className="text-muted-foreground hover:text-primary transition-colors">
                <X size={18} />
              </button>
            </div>
            ...
            <div ref={scrollRef} className="h-[320px] overflow-y-auto p-4 space-y-3">
              {messages.map((m, i) => (
                <div key={i} className={`flex gap-2.5 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                  <span className={`w-7 h-7 rounded-lg shrink-0 flex items-center justify-center ${m.role === "user" ? "bg-primary/20" : "bg-primary/20"}`}>
                    {m.role === "user" ? <span className="text-[11px] font-bold text-primary">U</span> : <Bot size={14} className="text-primary" />}
                  </span>
                  <div className={`max-w-[80%] rounded-xl px-3.5 py-2.5 text-sm leading-relaxed ${m.role === "user" ? "bg-primary/20 text-foreground" : "bg-secondary text-foreground"}`}>
                    {m.text}
                  </div>
                </div>
              ))}
              {loading && (
                <div className="flex gap-2.5">
                  <span className="w-7 h-7 rounded-lg shrink-0 flex items-center justify-center bg-primary/20"><Bot size={14} className="text-primary" /></span>
            ...
            {messages.length <= 1 && (
              <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                {SUGGESTIONS.map((s) => (
                  <button key={s} onClick={() => send(s)} className="text-[11px] px-2.5 py-1.5 rounded-full bg-secondary border border-border text-muted-foreground hover:border-primary hover:text-primary transition-colors">
                    {s}
                  </button>
                ))}
              </div>
